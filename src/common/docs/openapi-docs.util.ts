...
export function setupOpenApiDocs(
  ...
  const posDocument = buildAudienceDocument(
    ...
    [
      ...
      '```',
      '',
      '### Gestão da rede pelo terminal (DISTRIBUIDOR)',
      '```',
      'POST  /api/pos/vendedores          — Cadastrar vendedor na própria rede',
      'GET   /api/pos/vendedores          — Conferir os vendedores da rede',
      'POST  /api/pos/maquininhas         — Cadastrar maquininha de cartão',
      'PATCH /api/pos/maquininhas/:id     — Trocar o vendedor do aparelho, inativar',
      '```',
      '',
      'A rede sai sempre do token — nunca do corpo ou da query —, então um',
      'distribuidor não alcança a rede de outro. O perfil VENDEDOR recebe 403',
      'nessas rotas, com uma exceção:',
      '',
      '```',
      'GET   /api/pos/maquininhas         — VENDEDOR + DISTRIBUIDOR',
      '```',
      '',
      'O vendedor precisa dessa lista para escolher em qual aparelho passou a',
      'venda; ele enxerga apenas a maquininha atribuída a ele, e o distribuidor',
      'enxerga o parque inteiro da rede.',
      '',
      '### Maquininha na venda',
      'Informar `maquininhaId` em `POST /api/pos/vendas` e em',
      '`POST /api/pos/capital-sena/vendas` é opcional e exclusivo do POS: nos',
      'demais canais o campo não existe e a coluna fica nula. O aparelho precisa',
      'estar ATIVO e ao alcance do operador, senão a venda responde 404.',
      '',
      '### Autenticação',
